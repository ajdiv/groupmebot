...
async function getAwards(gmeGroupId: number, memberArr: any[]): Promise<string> {

  var time = getTodayAndTomorrow();
  var begin = time[0];
  var end = time[1];

  const results = await DailyUserPostCounter.find({
    gmeGroupId: gmeGroupId,
    date: { $gte: begin, $lt: end }
  }).sort({ messageCount: -1 });
  // Note: there will always be a result at this point,
  //since the user would have had to /awards to get to this point
  return Promise.resolve(createAwardsObj(results, memberArr));
}

function createAwardsObj(awardArr: any, memberArr: any[]): string {
  var result = "Number of Messages:\n    ";
  var resultArr: any[] = [];

  awardArr.forEach((person: any) => {
